...
#ifdef WANT_DOSTE
extern void doste_init(); /* [Nick] */
extern int doste_d(void*);
extern int doste_oid(char*);
#endif /* WANT_DOSTE */

/* basecontext is a part of the doste implementation, but taking the definition of this out
   would involve a lot of ifdef's, since it is used in many places.  Therefore leaving the
   definition in, but this variable will (should!) be left at the initial value if not
   WANT_DOSTE [Ash] */
int basecontext = 0;
...
int main(int, char *[]); /* change type from void to int due to gcc 2.8.1 */
...
*/
	...
	/* If interrupted is already set, don't do any work: skip to the end to
	 deal with it */
	if (!interrupted) {
    /* In Meurig & Ant's Sudoku model for their workshop in Paris in Aug
       2010, for some reason, on the Mac, the call to 'update' here never
       returns, and tkeden then stops responding to input.  This only seems to
       occur when we click on a Tk button created via %angel.  I can't see what
       is wrong with the input that %angel is giving to Tcl/Tk, and I don't know
       how to debug the internals of Tcl/Tk state to see why the freeze is
       occurring.  Commenting out the 'update' line below makes this model work,
       so let's do that for the time being.  Note that there is some advice in
       The Tcler's Wiki: "'update' considered harmful"
       <http://www2.tcl.tk/1255>.  A complicated solution is given at
       <http://www2.tcl.tk/1526>, but a comment there says "... It seems to me
       that this operation should be explicitly supported by the event loop.
       This is precisely the kind of obscure yak shaving that makes Tcl hard to
       get up to speed on".  I agree... reengineering the tkeden / Tcl event
       loop will have to be a job for another day.  [Ash, Aug 2010]. */
...
int main(int argc, char *argv[]) { /* MAIN PROGRAM */
...
#ifdef WANT_DOSTE
	doste_init();
	basecontext = doste_d(doste_oid("base"));
#endif /* WANT_DOSTE */
	...
	/* no longer required / can't get this to link [Ash] */
