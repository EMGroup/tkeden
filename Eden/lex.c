...
#ifdef WANT_DOSTE
extern void *doste_oid(char *);
extern int doste_d(void*);
extern void *doste_edenoid(int,int,int,int);
extern void *doste_lookup(void*,void*);
#endif /* WANT_DOSTE */
	...
	if ((i = lookup_local(yytext))) {	/* local variable ? */
	    ...
	    if (builtin_ft_check(yytext) == 0) {
		 ...
		 appAgentName, append_agentName, append_NoAgentName); */
	       if (*agentName != 0 && appAgentName > 0 && append_agentName > 0
...
void notationPushPop(int direction) {
  ...
  /* In ttyeden, eden.eden is not loaded, and this causes the "...eden.eden
     loaded correctly?" error to appear if execute() is performed in ttyeden.
     execute() should still work in ttyeden, however, so don't do any work here
     if we are ttyeden [Ash] */
#ifdef TTYEDEN
  return;
#endif
...
/* Debian bug #105231 now fixed: see http://bugs.debian.org/cgi-bin/bugreport.cgi?bug=105231 so commenting this out [Ash, also someone else previously] */
    /* rl_expand_prompt(""); */	/* work around bug in readline4-4.2 reported to Debian as bug #105231 */
    line_read = readline("");
  }
